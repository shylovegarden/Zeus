    ...
    fn lower_expr(&mut self, e: &Expression) -> ValueId {
        match e {
            ...
            Expression::Identifier(name) => {
                let secret = self.secret_names.contains(name) || self.env.get(name).map_or(false, |&v| self.values[v].secret);
                let nondet = self.env.get(name).map_or(false, |&v| self.values[v].nondet);
            ...
            Expression::Infix { left, operator, right } => {
                ...
                if operator=="Slash" && (self.sec(l)||self.sec(r)) {
                    self.leaks.push(format!("fn {}: secret-dependent division -> variable-time instruction [UNMITIGATED]", self.fname));
                }
            ...
            _ => self.push(ZInst::Opaque, ZType::Unknown, false, false),
        }
    }
