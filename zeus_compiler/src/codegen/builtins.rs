...
impl CCodegen {
    ...
    pub(crate) fn lower_math_builtin(&self, name: &str, a: &[String]) -> Option<String> {
        match (name, a.len()) {
            ...
            // String builtins
            ("string_len", 1) | ("strlen", 1) => Some(format!("((uint64_t)strlen({}))", a[0])),
            ("string_eq", 2)  => Some(format!("(strcmp({}, {}) == 0)", a[0], a[1])),
            // I/O builtins — read_file returns a heap-borrowed arena pointer (const char*)
            ("read_file", 1) => Some(format!(
                "({{ FILE* _rf = fopen({path}, \"r\"); const char* _rb = NULL; \
                if (_rf) {{ fseek(_rf, 0, SEEK_END); long _rsz = ftell(_rf); fseek(_rf, 0, SEEK_SET); \
                char* _rbuf = (char*)__zeus_arena_alloc((size_t)_rsz + 1); \
                fread(_rbuf, 1, (size_t)_rsz, _rf); _rbuf[_rsz] = '\\0'; fclose(_rf); _rb = _rbuf; }} _rb; }})",
                path = a[0])),
            ("write_file", 2) => Some(format!(
                "({{ FILE* _wf = fopen({path}, \"w\"); int _wok = 0; \
                if (_wf) {{ fputs({data}, _wf); fclose(_wf); _wok = 1; }} _wok; }})",
                path = a[0], data = a[1])),
            // Vec helpers (operate on ZeusVec struct)
            ("vec_len", 1)      => Some(format!("((uint64_t)({}).len)", a[0])),
            ("vec_cap", 1)      => Some(format!("((uint64_t)({}).cap)", a[0])),
            ("vec_is_empty", 1) => Some(format!("(({}).len == 0)", a[0])),
            _ => None,
        }
    }
