...
#line 1 "test_verify_fail.zs"
void process_data(double throttle) {
    if (!((throttle <= 100))) {
        fprintf(stderr, "[ZEUS PANIC]: Zeus Runtime Verification Failure at test_verify_fail.zs:%%d: Constraint '%s' violated\n", __LINE__, "(throttle <= 100)");
        exit(1);
    }
